...
mnist = MNISTDataModule(os.getcwd(), batch_size=64, num_workers=2, pin_memory=True, persistent_workers=True)
...
trainer = pl.Trainer(max_epochs=5, default_root_dir=os.getcwd(), strategy=my_strategy, accelerator="gpu", devices=1, log_every_n_steps=25, callbacks=[TimeEpochCallback()], logger=neptune_logger, precision=32, enable_progress_bar=False) #gpus=1
